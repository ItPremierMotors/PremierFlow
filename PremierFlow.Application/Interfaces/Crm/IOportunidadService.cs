...
namespace PremierFlow.Application.Interfaces.Crm
{
    public interface IOportunidadService
    {
        ...
        Task<ApiResponse<List<OportunidadDTO>>> GetAllAsync(int? sucursalId); //obtenemos todas las oportunidades, opcionalmente filtradas por sucursal
        ...
        Task<ApiResponse<List<OportunidadDTO>>> GetByEtapaAsync(EtapaOportunidad etapa, int? sucursalId); //obtenemos las oportunidades filtradas por su etapa y opcionalmente por sucursal
        ...
        Task<ApiResponse<bool>> AvanzarEtapaAsync(int oportunidadId, string usuarioId); //avanzamos la etapa de una oportunidad
        Task<ApiResponse<bool>> RetrocederEtapaAsync(int oportunidadId, string usuarioId); //retrocedemos la etapa de una oportunidad
        Task<ApiResponse<bool>> CerrarGanadaAsync(CerrarOportunidadDTO dto, string usuarioId); //cerramos una oportunidad como ganada
        ...
        Task<ApiResponse<bool>> VincularVehiculoAsync(int oportunidadId, int vehiculoId, string usuarioId); //vinculamos un vehículo a una oportunidad
    }
}
